...
func (s *Server) adminGatewaySettings(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut, http.MethodPatch:
		...
		if payload.AllowLANManagement == nil {
			writeError(w, http.StatusBadRequest, "invalid_request", "allow_lan_management is required", useClientRequestID(r))
			return
		}
		settings := store.GatewaySettings{AllowLANManagement: *payload.AllowLANManagement}
